...
class DungeonGame(BaseGame):
    ...
    def __init__(self, difficulty: str = 'normal') -> None:
        ...
        self.player_pos = [1, 1]
        self.dungeon_map: List[List[int]] = self._generate_level()
        self.input_handler = get_safe_input_handler()

    def _generate_level(self) -> List[List[int]]:
        size = 10
        ...
        for r, row in enumerate(self.dungeon_map):
            ...
            for c, cell in enumerate(row):
                if [r, c] == self.player_pos:
                    ...
                    line += f"{C_RED}E{C_RESET} "
                elif cell == 3:
                    line += f"{C_GREEN}H{C_RESET} "
                elif cell == 4:
...
def play_dungeon(difficulty: str = 'normal') -> dict:
