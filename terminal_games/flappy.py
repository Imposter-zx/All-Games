...
class FlappyGame(BaseGame):
    ...
    def __init__(self, difficulty='normal'):
        ...
        self.bird_pos = [BOARD_HEIGHT // 2, 10]
        self.velocity = 0
        self.gravity = 0.6
        self.jump_strength = -1.5
        self.pipes = [] # List of {'x': int, 'gap_y': int}
        self.pipe_width = 4
        self.pipe_gap = 6
        self.frame_count = 0
    ...
    def _render(self):
        ...
        for r in range(BOARD_HEIGHT):
            ...
            for c in range(BOARD_WIDTH):
                char = grid[r][c]
                if char == '►': line += f"{C_YELLOW}{char}{C_RESET}"
                elif char == '█': line += f"{C_GREEN}{char}{C_RESET}"
                else: line += char
            line += f"{C_CYAN}║{C_RESET}"
            print(line)
        print(f"{C_CYAN}╚{'═' * BOARD_WIDTH}╝{C_RESET}")
    ...
    def _update_game_state(self):
        ...
        for pipe in self.pipes:
            ...
            if pipe['x'] + self.pipe_width == self.bird_pos[1]:
                ...
                beep("correct")
                if self.score == 10: self.unlock_achievement("flappy_10", "Birdie")
                if self.score == 50: self.unlock_achievement("flappy_50", "Ace Flyer")
...
def play_flappy(difficulty='normal'):
