...
class AsteroidsGame(BaseGame):
    ...
    def __init__(self, difficulty='normal'):
        ...
        if difficulty == 'easy':
            self.spawn_rate = 20
            self.asteroid_speed_max = 0.3
        elif difficulty == 'hard':
            self.spawn_rate = 8
            self.asteroid_speed_max = 0.6
        else:
            self.spawn_rate = 12
            self.asteroid_speed_max = 0.4
    ...
    def spawn_asteroid(self):
        """Spawn a new asteroid from a random edge."""
    ...
    def update(self):
        ...
        # Update ship position with momentum
    ...
    def render(self):
        """Draw the game state."""
        lines = []
        lines.append(f" SCORE: {C_YELLOW}{self.score}{C_RESET} | SPEED: {abs(self.vx)+abs(self.vy):.1f} | DIFFICULTY: {self.difficulty.upper()}")
        ...
        print(f"\n{C_WHITE}   [ARROWS] Thrust/Rotate  [SPACE] Shoot  [Q] Quit{C_RESET}")
        ...
        try:
            while not self.game_over:
                ...
                if key == 'q': 
                    ...
                    dx, dy = 0.0, 0.0
                    if self.direction == 'up': dy = -1.0
                    elif self.direction == 'down': dy = 1.0
                    elif self.direction == 'left': dx = -1.0
                    elif self.direction == 'right': dx = 1.0
                    ...
                    if direction:
                        ...
                        # Thrust
                        if direction == 'up': self.vy = max(-0.8, self.vy - 0.2)
                        elif direction == 'down': self.vy = min(0.8, self.vy + 0.2)
                        elif direction == 'left': self.vx = max(-0.8, self.vx - 0.2)
                        elif direction == 'right': self.vx = min(0.8, self.vx + 0.2)
...
def play_asteroids(difficulty='normal'):
